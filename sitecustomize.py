"""Repository-wide last-line duplicate mail protection for morning news jobs.

Python imports sitecustomize automatically when the repository root is on sys.path.
Only known morning-news subjects are guarded; unrelated mail is untouched.
The check is fail-open so an IMAP outage never blocks a legitimate delivery.
"""
...
def _search_token(subject: str) -> str:
    # Direct-site subjects end in a count that can legitimately change between retries.
