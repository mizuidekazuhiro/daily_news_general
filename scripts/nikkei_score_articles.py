...
def split_keywords(raw: str) -> list[str]:
    """Split comma/newline/pipe separated rules and standalone OR tokens only.

    The previous implementation split every substring "or" (case-insensitive),
    corrupting words such as "north" and "Hormuz" into one-letter keywords.
    """
    if not raw:
        return []
    text = str(raw).replace("\r\n", "\n")
    text = re.sub(r"(?<![A-Za-z0-9])OR(?![A-Za-z0-9])", "\n", text, flags=re.IGNORECASE)
    text = re.sub(r"[\n,、;|　]+", "\n", text)
    return [t.strip() for t in text.split("\n") if t.strip()]


def _contains_keyword(target: str, keyword: str) -> bool:
    """Match Japanese keywords by substring and ASCII keywords on token boundaries.

    Boundary matching prevents short rules such as "us" from matching "USEN".
    Phrases still match naturally, e.g. "data center" or "capacity expansion".
    """
    kw = str(keyword or "").strip().lower()
    if not kw:
        return False
    text = str(target or "").lower()
    if re.fullmatch(r"[ァ-ヿー]+", kw):
        # Avoid country-name substring collisions such as タイ in タイプ or インド in インドネシア.
        return re.search(r"(?<![ァ-ヿー])" + re.escape(kw) + r"(?![ァ-ヿー])", text) is not None
    if not re.fullmatch(r"[\x00-\x7f]+", kw):
        return kw in text

    left = r"(?<![a-z0-9])" if kw[0].isalnum() else ""
    right = r"(?![a-z0-9])" if kw[-1].isalnum() else ""
    return re.search(left + re.escape(kw) + right, text, flags=re.IGNORECASE) is not None
...
def clean_text_for_scoring(text: str) -> str:
    ...
    drop_exact = {
        "共有", "文字サイズ", "小", "中", "大", "自動翻訳",
        "英文（システムによる自動翻訳）を表示する", "日経の記事利用サービス",
        "保存", "印刷 翻訳", "検索する", "その他", "［有料会員限定］", "[有料会員限定]",
    }
    drop_patterns = [
        r"朝夕刊や電子版ではお伝えしきれない情報をお届けします。?.*",
        r"企業での記事共有や会議資料への転載・複製.*",
        r".*注文印刷.*", r".*詳しくはこちら.*", r"^\d+文字$",
        r".*javascript:void\(0\).*", r"^その他javascript:void\(0\).*$",
    ]
    kept = []
...
def load_rules(token: str, rules_db_id: str, rule_types: set[str]) -> list[dict[str, Any]]:
    ...
    while True:
        ...
        resp = requests.post(
            ...
            headers=notion_headers(token), json=payload, timeout=60,
        )
...
def score_article(article: dict[str, Any], rules: list[dict[str, Any]], min_report_score: float) -> dict[str, Any]:
    ...
    for rule in rules:
        ...
        positive_hits = [kw for kw in rule.get("keywords", []) if _contains_keyword(target, kw)]
        negative_hits = [kw for kw in rule.get("negative_keywords", []) if _contains_keyword(target, kw)]
        tag = rule.get("tag_name", "")

        # NegativeKeywords are exception/suppression conditions for the rule.
        # They do not create a second penalty on their own.
        if positive_hits and negative_hits:
            for hit in negative_hits:
                negative_matches.append(f"{tag}:{hit}")
            score_breakdown.append({
                "tag": tag, "type": "suppressed_by_negative",
                "hits": positive_hits, "negative_hits": negative_hits, "weight": 0.0,
            })
            continue
        ...
        if positive_hits:
            ...
            score_breakdown.append({
                "tag": tag, "type": "positive", "hits": positive_hits,
    ...
    out.update({
        ...
        **derive_tags_from_rules(tags_by_type),
    })
    return out


def select_report_articles(
    articles: list[dict[str, Any]], selection_mode: str, min_report_score: float,
    top_rank: int, include_ties: bool,
) -> tuple[list[dict[str, Any]], float | None, str]:
    mode = (selection_mode or "top_importance_rank").strip().lower()
    candidates = [
        a for a in articles
        if a.get("importance_score") is not None and not bool(a.get("exclude_candidate"))
    ]
    eligible = [a for a in candidates if float(a.get("importance_score", 0)) >= min_report_score]

    if mode == "threshold":
        ordered = sorted(
            eligible,
            key=lambda a: (
                float(a.get("importance_score", 0)), int(a.get("priority", 0)),
                str(a.get("issue_date") or ""), str(a.get("source_title") or a.get("page_title") or ""),
            ), reverse=True,
        )
        return ordered, min_report_score, "threshold"
    ...
    if include_ties:
        return [a for a in ordered if float(a.get("importance_score", 0)) >= cutoff_score], cutoff_score, "top_importance_rank_with_ties"
...
def main() -> int:
    ...
    for item in inventory:
        if item.get("status") == "existing_in_notion":
            ...
            match = next((x for x in fetched_articles if x.get("url") == item.get("url")), None)
            if match:
                backfilled_articles.append(match)
    ...
    selected, cutoff_score, selection_mode_used = select_report_articles(
        scored, report_selection_mode, min_report_score, report_top_rank, report_include_ties,
    )
    ...
    for a in scored:
        ...
        if included:
            ...
            if a.get("exclude_candidate"):
                a["report_excluded_reason"] = "exclude_candidate"
            elif float(a.get("importance_score", 0)) < min_report_score:
                a["report_excluded_reason"] = "below_min_importance_score"
            else:
    ...
    summary = {
        ...
        "report_min_importance_score": min_report_score,
        "report_top_rank": report_top_rank,
        "report_include_ties": report_include_ties,
        "report_candidate_count": eligible_count,
