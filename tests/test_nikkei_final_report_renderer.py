...
def test_full_issue_article_list_is_not_rendered():
    rep = {"today_key_message": "k", "integrated_insights": ["i"], "article_sections": []}
    html = render_final_report_html(
        Path("templates/nikkei_final_report_email.html"),
        rep,
        "2026-05-06",
        all_articles=[{"title": "all1", "url": "https://all1", "summary": "xx"}],
    )
    assert "取得記事一覧" not in html
    assert "all1" not in html
