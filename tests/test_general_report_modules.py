...
def test_selection_cases():
    selected, _ = select_articles(mk([10, 8, 6, 5, 4, 3]), SelectionConfig())
    assert [x["importance_score"] for x in selected] == [10, 8, 6, 5]

    selected, _ = select_articles(mk([2, 2, 2, 1, 1, 1]), SelectionConfig())
    assert selected == []

    tied, _ = select_articles(mk([10, 8, 6, 5, 5, 5]), SelectionConfig(top_rank=5, include_ties=True))
    assert len(tied) == 6

    exact, _ = select_articles(mk([10, 8, 6, 5, 5, 5]), SelectionConfig(top_rank=5, include_ties=False))
    assert len(exact) == 5
...
def test_final_synthesis_and_hash():
    ...
    rep = {
        ...
        "watchlist": [],
        "article_sections": [{"ref_id": "A1", "url": "u"}],
    }
